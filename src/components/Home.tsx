...
const Home = (props: any) => {
  ...
  const handleLogout = () => {
    fetch(`http://34.149.150.192:80/admin_api/logout`, {
      method: 'POST',
      headers: {
        Authorization: token
      },
  ...
  const deleteUser = async (email: string): Promise<any> => {
    const response = await fetch(`http://34.149.150.192:80/admin_api/user`, {
  ...
  const getUsers = async (): Promise<User[]> => {
    const response = await fetch(`http://34.149.150.192:80/admin_api/all_users`, {
      method: 'GET',
      headers: {
        Authorization: token
      }
