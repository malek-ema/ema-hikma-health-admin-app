...
const PatientExport = (props: any) => {
  ...
  const getPatients = async (): Promise<User[]> => {
    const response = await fetch(`http://192.168.0.108:8080/admin_api/all_patients`, {
      method: 'GET',
      headers: {
        Authorization: token
      }
  ...
  const searchPatients = async () => {
    fetch(`http://192.168.0.108:8080/admin_api/search_patients`, {
      method: 'POST',
      headers: {
        Authorization: token
      },
  ...
  const handleExportAllPatients = () => {
    fetch(`http://192.168.0.108:8080/admin_api/export`, {
      method: 'POST',
      headers: {
        Authorization: token
      }
  ...
  const handleExportSinglePatient = (patientId: string) => {
    fetch(`http://192.168.0.108:8080/admin_api/export_patient`, {
      method: 'POST',
      headers: {
        Authorization: token
      },
